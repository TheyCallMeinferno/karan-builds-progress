      ...
      <div className="container mx-auto px-6">
        ...
        <div className="text-center mb-16 animate-fade-in">
          <h2 className="text-4xl lg:text-5xl font-bold text-foreground mb-4">
            My <span className="text-transparent bg-clip-text gradient-primary">Skills</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {skillCategories.map((category, index) => (
            <Card 
              key={category.title}
              className="shadow-elegant hover:shadow-glow transition-all duration-500 transform hover:scale-105 animate-slide-up group"
                ...
                <div className="flex flex-wrap gap-3">
                  {category.skills.map((skill, skillIndex) => (
                    <Badge 
                      key={skill}
                      variant="secondary"
                      className="px-4 py-2 text-sm font-medium hover:bg-primary hover:text-primary-foreground transition-all duration-300 cursor-default transform hover:scale-105"
                      style={{ 
                ...
                <div className="mt-6 pt-4 border-t border-border">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-primary font-medium">
                      {category.title.includes('Programming') ? 'Intermediate' : 
                       category.title.includes('Development') ? 'Growing' :
                       category.title.includes('Technical') ? 'Proficient' : 'Advanced'}
                    </span>
                  </div>
                  <div className="mt-2 h-2 bg-secondary rounded-full overflow-hidden">
                    <div 
                      ...
                      style={{ 
                        width: category.title.includes('Programming') ? '70%' : 
                               category.title.includes('Development') ? '65%' :
                               category.title.includes('Technical') ? '75%' : '85%',
